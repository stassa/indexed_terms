...
vectors and matrices (a.k.a. "tensors").
...
can then be re-implemented as foreign language predicates, if
...
%	using append/3 to concatenate the two terms apply.
%
iterm_extend(T,A,T_):-
...
%	stride may not result in exactly opposite intervals:
%	==
...
%	the numbers in an interval generated by interval/4.
%
sequence(F,I,J,K,T):-
	findall(V
	       ,interval(I,J,K,V)
...
%
%	@tbd Needs documentation and examples. In the meantime, see
...
%
%	window/6 has many and varied use cases. For examples of usage,
%	see below, after Intuition and Formal Definition.
%
%	Intuition
%	---------
%
...
%	==
%
%	Simiarly, the following example shows how window/6 can be used
%	to reverse a term:
%	==
%       ?- sequence(v,1,5,1,S), window(S,5,1,5,1,Ws).
%       S = v(1, 2, 3, 4, 5),
%       Ws = [v(5, 4, 3, 2, 1)].
%	==
%
